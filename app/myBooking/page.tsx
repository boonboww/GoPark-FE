...
export default function MyBookingPage() {
  ...
  const loadBookings = async () => {
    try {
      ...
      if (response.status === "success" && response.data) {
        ...
        formattedBookings.sort((a, b) => new Date(b.startTime).getTime() - new Date(a.startTime).getTime());
  ...
  const filteredBookings = useMemo(() => {
    return bookings.filter(booking => {
      ...
      if (filterStatus === "active") return ["pending", "confirmed", "active"].includes(booking.status);
  ...
  const getStatusConfig = (status: string) => {
    switch (status) {
      case "active":
      case "confirmed":
        return { label: "Đang hoạt động", color: "text-green-600 bg-green-50 border-green-200", icon: CheckCircle2 };
      case "pending":
        return { label: "Chờ xác nhận", color: "text-yellow-600 bg-yellow-50 border-yellow-200", icon: Timer };
      case "completed":
        return { label: "Hoàn thành", color: "text-blue-600 bg-blue-50 border-blue-200", icon: CheckCircle2 };
      case "cancelled":
        return { label: "Đã hủy", color: "text-red-600 bg-red-50 border-red-200", icon: XCircle };
      default:
        ...
        <div className="flex flex-col md:flex-row">
          ...
          >
            <Image
              ...
              onError={() => setImageErrors(prev => new Set([...prev, booking.id]))}
            />
            <div className="absolute top-3 left-3 md:hidden">
              <Badge className={cn("font-medium border shadow-sm", statusConfig.color)}>
            ...
            <div>
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="hidden md:block">
                  <Badge className={cn("font-medium border px-3 py-1", statusConfig.color)}>
                    <StatusIcon className="w-3 h-3 mr-1" />
                    {statusConfig.label}
                  </Badge>
                </div>
              ...
              <div className="grid grid-cols-2 gap-y-3 gap-x-6 mt-4">
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{booking.plateNumber}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">{booking.zone}-{booking.spotNumber}</p>
                  </div>
                </div>
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <p className="text-xs text-gray-500 font-medium">Tổng phí</p>
                    <p className="text-sm font-semibold text-green-600">{booking.fee}</p>
                  </div>
                </div>
              </div>
            </div>
        ...
        <div className="max-w-5xl mx-auto px-4 py-8">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <p className="text-gray-500 mt-1">Xem và quản lý lịch sử đỗ xe của bạn</p>
            </div>
            <Button
          ...
          <div className="space-y-4">
            {filteredBookings.length === 0 ? (
              <div className="text-center py-16 bg-white rounded-2xl border border-dashed">
                ...
                <h3 className="text-lg font-semibold text-gray-900 mb-1">Không tìm thấy đặt chỗ nào</h3>
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
              ...
              <div className="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
                <div className="flex items-center gap-3 mb-4 text-red-600">
                  ...
                  <h2 className="text-lg font-bold text-gray-900">Xác nhận hủy đặt chỗ</h2>
                </div>
