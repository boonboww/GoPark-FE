...
export default function FindParkingPage() {
  ...
  useEffect(() => {
    ...
    switch (filters.sortBy) {
      ...
      case "slots-desc":
        filtered.sort(
          (a, b) => getTotalSlots(b.zones) - getTotalSlots(a.zones)
        );
        break;
      case "slots-asc":
        filtered.sort(
          (a, b) => getTotalSlots(a.zones) - getTotalSlots(b.zones)
        );
        break;
      case "distance":
        if (userLocation) {
        ...
        {filteredLots.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-gray-500 mb-6 max-w-md mx-auto">
              Thử điều chỉnh bộ lọc hoặc từ khóa tìm kiếm để có kết quả phù hợp
              hơn
            </p>
            <Button
              onClick={clearFilters}
            ...
            {filteredLots.map((lot) => (
              ...
              >
                ...
                <div
                  className={`relative ${
                    viewMode === "list"
                      ? "sm:w-1/2 h-56 sm:h-auto flex-shrink-0"
                      : "h-56 w-full"
                  }`}
                >
                  {lot.avtImage ? (
